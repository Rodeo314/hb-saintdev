...
static void do_job(hb_job_t *job)
{
        ...
        if (job->list_filter != NULL && hb_list_count(job->list_filter) > 0)
        {
            while (hb_list_count(job->list_filter) > num_cpu_filters)
            {
                ...
                switch (filter->id)
                {
                    ...
                    // note: we also drop QSV filters but we'll add them later
                    default:
                        hb_list_rem(job->list_filter, filter);
                        hb_filter_close(&filter);
                        break;
    ...
    else if (job->list_filter != NULL && hb_list_count(job->list_filter) > 0)
    {
        i = 0;
        // we're not decoding with QSV, so remove QSV filters
        while (i < hb_list_count(job->list_filter))
        {
            hb_filter_object_t *filter = hb_list_item(job->list_filter, i);
            switch (filter->id)
            {
