                                ...
                                const size_t nal_unit_size)
{
    ...
    if (buf != NULL)
    {
        hb_log("hb_nal_unit_write_isomp4:       NAL with type %02"PRIu8" and size %5lu (%5"PRIu32", %lu bytes)", nal_unit[0] & 0x1f, nal_unit_size, nalu_length, sizeof(nalu_length));//debug
