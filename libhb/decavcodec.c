...
static int decavcodecvInit( hb_work_object_t * w, hb_job_t * job )
{
    ...
    if ( pv->title->opaque_priv )
    {
        ...
        if (hb_qsv_decode_is_enabled(job))
        {
            ...
            {
                hb_log("decavcodecvInit: hb_qsv_decode_setup failed");
                return 1;
            }
            else
            {
        ...
        if (pv->qsv_decode)
        {
            hb_qsv_decode_init(pv->context, &pv->qsv_config, job->qsv_async_depth);
#ifdef USE_QSV_PTS_WORKAROUND
                            ...
                            hb_buffer_t ** buf_out )
{
    ...
    *buf_out = NULL;
    ...
    if ( !pv->video_codec_opened )
    {
        ...
        if (pv->qsv_decode)
        {
            hb_qsv_decode_init(pv->context, &pv->qsv_config, pv->job->qsv_async_depth);
#ifdef USE_QSV_PTS_WORKAROUND
            pv->qsv_pts_list = hb_list_init();
#endif
        }
